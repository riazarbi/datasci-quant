...
for (i in seq_along(1:length(stocks))) {
  stock <- stocks[i]
  json_query <- GET(paste0("https://www.alphavantage.co/query?function=INCOME_STATEMENT&symbol=",
                           stock,
  ...
  if(!is.null(result$annualReports)) {
    ...
    update_minio_dataset(annual_reports_list, 
                         prefix = paste0("datasci-quant/alphavantage/annual_reports/income_statement/",
                                         stock), 
  ...
  if(!is.null(result$quarterlyReports)) {
    ...
    update_minio_dataset(quarterly_reports_list, 
                         prefix = paste0("datasci-quant/alphavantage/quarterly_reports/income_statement/",
                                         stock), 
                         key_cols = "fiscalDateEnding",
                         s3_key = s3_key,
                         s3_secret = s3_secret, 
